...
def pack_FI_metadata(self):
    ...
    headerData.append(self._no_active_beams)
    data_type += 'l'
...
def pack_FI_data(self, data):
    ...
    for BESecIndex in range(self._no_active_beams):
        ...
        data_type += 'l'
        pafData.append(BESecNum)
        data_type += 'l'
        pafData.append(self._no_channels)
        #data_type += '1024f'
        data_type += str('%sf' % self._no_channels)

        pafData.extend(channelData[dataPointer:dataPointer+channels])
        dataPointer += self._no_channels

    return(data_type, pafData)
...
class CaptureData(Thread):
    ...
    def start_capture(self):
        ...
        print "start check...", self._stop_event.is_set()

    def stop_capture(self):
        self._stop_event.set()
        print "stop check...", self._stop_event.is_set()
	...
	while True:
            if not self._stop_event.is_set():
               try:
                    ...
                    print "length self._data: ", len(self._data)
...
class AggregateData():
    ...
    def start_aggregating(self, data_to_process):
        self._count += 1
        print "data length: ", len(data_to_process),
...
class SendToFW(Thread):
    ...
    def read_data(self):
        stream, data_from_queue = data_Queue.get()
        return stream, data_from_queue
